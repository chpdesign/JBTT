...
public class UrlEncodedParser extends DataParser {
    ...
    public static Map<String, String[]> parse(String urlEncoded) {
        try {
            return new UrlEncodedParser().parse(new ByteArrayInputStream(urlEncoded.getBytes("ISO-8859-1")));
    ...
    public Map<String, String[]> parse(InputStream is) {
        try {
            ...
            if(!forQueryString) {
                params.put("body", new String[] {new String(data, "ISO-8859-1")});
            }
            ...
            while (ix < data.length) {
                ...
                switch ((char) c) {
                    case '&':
                        value = new String(data, 0, ox, "ISO-8859-1");
                        if (key != null) {
                    ...
                    case '=':
                        if (key == null) {
                            key = new String(data, 0, ox, "ISO-8859-1");
                            ox = 0;
                        } else {
                            data[ox++] = c;
                        }
